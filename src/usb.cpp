            ...
            if ((endpointQueueHeadTx->nextPointer & 1) == 0) {
                ...
                while ((address & 1) == 0) {
                    ...
                    if (td->status & (1 << 7)) { // Active
                        u8* data = (u8*)callbacks.MemoryMap(td->bufferPointer0);
                        if (i == 0) {
...
void USB::HandleDataWrite(int ep, int interval, const u8* data, std::size_t length)
{
    callbacks.Lock();
    ...
    if ((endpointQueueHeadRx->nextPointer & 1) == 0) {
        ...
        if ((address & 1) == 0) {
            ...
            length = std::min(td->totalBytes, (u16)length);
            td->status = 0;
...
void USB::HandleDataRead(int ep, int interval, std::size_t limit, std::function<void(const u8*, std::size_t)> callback)
{
