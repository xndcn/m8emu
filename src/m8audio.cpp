...
struct __attribute__ ((packed)) _AudioStreamMod
{
    _AudioStream root;
    u8 num_inputs;             // 0x18
    u8 padding[3];
...
static_assert(offsetof(_AudioStreamMod, num_inputs) == 0x18);
static_assert(offsetof(_AudioStreamMod, destination_list_ptr) == 0x1c);
static_assert(offsetof(_AudioStreamMod, inputQueue_ptr) == 0x20);
...
void M8AudioProcessor::ParseConnections(u32 first_update)
{
    ...
    while (ptr) {
        ...
        if (stream->destination_list_ptr) {
            ...
            ptr = ((_AudioStreamMod*)stream)->destination_list_ptr;
        }
        while (ptr) {
